...
def AtelierD3WebUI(client, address: str = None, port: int = None, browser: bool = True, upload_size: str = "4MB",
                   public: bool = False, limit: int = 1, quiet: bool = True):
    """
    Start Atelier D3 WebUI with all features.
    ...
    try:
        ...
        with gr.Blocks(title=f"Atelier D3 Client {version}", css=css, analytics_enabled=False, theme=system_theme, fill_height=True).queue(default_concurrency_limit=limit) as demo:
            
            with gr.Row():
                with gr.Column(scale=1):
                    Markdown(f"## <br><center>Atelier Client D3 {version}")
                    Markdown(f"<center>Copyright (C) 2023-{datetime.now().year} Ikmal Said. All rights reserved")
            
            with gr.Tab("D3 Image Generator"):
